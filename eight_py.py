...
class Api():	
    ...
    def next_song(self):
        ...
        @return same thing as start_playback except for the next track, unless we hit an error in which case we return
        None
        """
        logging.info("going to next song in the set")
        try:
            return self.current_set.get_stream_data(first_time=False)
        except TrackAccessException:
            return None
